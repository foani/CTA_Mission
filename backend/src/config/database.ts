...
import { AppDataSource, dataSourceOptions } from './typeorm';
import { logger } from '../utils/logger';

// PostgreSQL 설정을 위한 타입 단언
...
class DatabaseManager {
  ...
  public async initialize(): Promise<DataSource> {
    try {
      ...
      const pgOptions = dataSourceOptions as PostgresDataSourceOptions;
      
      logger.info('데이터베이스 연결을 시도합니다...');
      logger.info(`호스트: ${pgOptions.host}:${pgOptions.port}`);
      logger.info(`데이터베이스: ${pgOptions.database}`);
  ...
  public async healthCheck(): Promise<{
    ...
    try {
      ...
      const pgOptions = dataSourceOptions as PostgresDataSourceOptions;
      const poolInfo = {
        totalConnections: pgOptions.extra?.max || 'unknown',
...
// AppDataSource alias for backward compatibility
export { AppDataSource } from './typeorm';
/**
 * 트랜잭션 실행 (간편 함수)
 */
